class RegistrationsController < ApplicationController
  ...
  def create
    ...
    if params[:advertiser].present?
      ...
      NotificationMailer.delay(run_at: 10.seconds.from_now).welcome_mail("advertiser",new_advertiser.id).deliver!
    else
      new_influencer = Influencer.new(influencer_params)
      activation_token = ApplicationController.new.generate_activation_token
      new_influencer.token = activation_token
      new_influencer.save!
      NotificationMailer.delay(run_at: 10.seconds.from_now).welcome_mail("influencer",new_influencer.id).deliver!
    end
